...
const handdrawnSprites = {
  ...
  // 認真樣子動畫：1-6 順著播（右鍵點擊時使用）
  serious: [
    'sprites/serious/serious_1.PNG',
    'sprites/serious/serious_2.PNG',
    'sprites/serious/serious_3.PNG',
    'sprites/serious/serious_4.PNG',
    'sprites/serious/serious_5.PNG',
    'sprites/serious/serious_6.PNG'
  ],
  // 被拖拽掙扎動畫：1-7 循環
  grasp: [
    'sprites/grasp/grasp_1.PNG',
    'sprites/grasp/grasp_2.PNG',
    'sprites/grasp/grasp_3.PNG',
    'sprites/grasp/grasp_4.PNG',
    'sprites/grasp/grasp_5.PNG',
    'sprites/grasp/grasp_6.PNG',
    'sprites/grasp/grasp_7.PNG'
  ],
  // 彈跳動畫（暫時使用第一幀）
...
let isManuallyPaused = false; // 追蹤是否為手動點擊"暫停移動"按鈕暫停

function loadSprite(...spritePaths) {
  return new Promise((resolve) => {
    function tryLoadSprite(index) {
      if (index >= spritePaths.length) {
        console.log('所有圖片載入失敗，使用預設圖片');
        resolve(defaultSprite); // 如果所有路徑都失敗，使用預設
        return;
      }
      
      const currentPath = spritePaths[index];
      console.log(`嘗試載入圖片 ${index}: ${currentPath}`);
      
      const img = new Image();
      img.onload = () => {
        console.log(`圖片載入成功: ${currentPath}`);
        resolve(currentPath);
      };
      img.onerror = () => {
        console.log(`圖片載入失敗: ${currentPath}`);
        tryLoadSprite(index + 1); // 嘗試下一個
      };
      img.src = currentPath;
    }
...
function animateFrames() {
  ...
  // 為不同狀態設置不同的動畫速度
  let frameInterval = 400; // 預設每400ms切換一幀
  
  if (currentState === 'awake') {
    frameInterval = 200; // awake 動畫放慢，每200ms切換一幀
  } else if (currentState === 'idle_sleeping') {
    frameInterval = 600; // 睡覺動畫更慢，每600ms切換一幀
  } else if (currentState === 'walking') {
    frameInterval = 150; // 走路動畫稍快，每150ms切換一幀
  } else if (currentState === 'grasp') {
    frameInterval = 80; // 掙扎動畫很快，每80ms切換一幀，表現緊張感
  }
...
function handleContextMenu(e) {
  ...
  // 檢查當前是否為 idle_sleeping 狀態
  if (currentState === 'idle_sleeping') {
    console.log('從 idle_sleeping 狀態喚醒，播放 awake 動畫');
    setPetState('awake');
    
    // awake 動畫播放一段時間後切換到 serious (8幀 * 200ms = 1600ms + 緩衝)
    setTimeout(() => {
      if (isContextMenuOpen) { // 確保選單仍然開啟
        console.log('awake 動畫完成，切換到 serious 狀態');
        setPetState('serious');
      }
    }, 1800); // 1.8秒後切換到 serious，讓用戶能清楚看到 awake 動畫
  } else {
    // 其他狀態直接使用 serious 動畫
    setPetState('serious');
  }
...
function initializePet() {
  ...
  petElement.addEventListener('mousedown', (e) => {
    ...
    if (e.button === 0) {
      ...
      // 記錄鼠標相對於桌寵中心的偏移
      window.dragOffsetX = e.clientX - petCenterX;
      window.dragOffsetY = e.clientY - petCenterY;
      
      console.log('Drag offset:', window.dragOffsetX, window.dragOffsetY);
  ...
  window.addEventListener('mousemove', (e) => {
    if (dragging) {
      // 計算桌寵應該移動到的位置（鼠標位置減去偏移量）
      const targetX = e.screenX - window.dragOffsetX - 64; // 64 是窗口一半寬度
      const targetY = e.screenY - window.dragOffsetY - 64; // 64 是窗口一半高度
      
      console.log('Mouse at:', e.screenX, e.screenY, 'Moving window to:', targetX, targetY);
      
      // 立即移動窗口，不使用節流
      if (window.electronAPI) {
        window.electronAPI.moveWindow(targetX, targetY);
      }
...
function setupIpcListeners() {
  ...
  if (window.electronAPI.onTogglePermanentPause) {
    window.electronAPI.onTogglePermanentPause((event, pauseState) => {
      ...
      if (pauseState) {
        // 手動暫停時使用 idle_sleeping 動畫
        isManuallyPaused = true;
        setPetState('idle_sleeping');
        console.log('桌寵已暫停，進入 idle_sleeping 狀態');
      } else {
  ...
  // 新增：監聽統一的暫停狀態更新
  if (window.electronAPI.onUpdatePauseState) {
    window.electronAPI.onUpdatePauseState((event, shouldPause) => {
      isTemporaryPaused = shouldPause;
      console.log('桌寵暫停狀態更新:', shouldPause ? '暫停' : '恢復');
      if (shouldPause) {
        // 如果是因為視窗開啟而暫停，使用 serious 動畫
        // 如果是因為右鍵選單而暫停，保持當前動畫不變
        if (!isContextMenuOpen && !isManuallyPaused) {
          setPetState('serious');
        }
      } else if (!isPaused) { // 只有在非永久暫停時才恢復移動
        // 如果不是手動暫停，才恢復到 walk 狀態
        if (!isManuallyPaused) {
          setPetState('walk');
        }
      }
  ...
  // 監聽強制恢復移動事件
  if (window.electronAPI.onForceResumeMovement) {
    window.electronAPI.onForceResumeMovement(() => {
      console.log('=== 強制恢復移動 ===');
      isPaused = false;
      isManuallyPaused = false;
      isTemporaryPaused = false;
      setPetState('walk');
      console.log('所有暫停狀態已重置，桌寵開始移動');
    });
  }
  ...
  if (window.electronAPI.onContextMenuClosed) {
    window.electronAPI.onContextMenuClosed(() => {
      ...
      if (!isPaused && !isTemporaryPaused) {
        ...
        console.log('不恢復移動，isPaused:', isPaused, 'isTemporaryPaused:', isTemporaryPaused, 'isManuallyPaused:', isManuallyPaused);
        // 如果是手動暫停，保持 idle_sleeping 狀態
        if (isManuallyPaused) {
          setPetState('idle_sleeping');
        }
      }
