...
// 跨平台兼容性：只在macOS上載入mac特定功能
let watchFrontmostApp, isDesktopProcess;
if (process.platform === 'darwin') {
  try {
    const macModule = require('./macFrontmost');
    watchFrontmostApp = macModule.watchFrontmostApp;
    isDesktopProcess = macModule.isDesktopProcess;
  } catch (error) {
    console.warn('Mac-specific features not available:', error.message);
    watchFrontmostApp = null;
    isDesktopProcess = null;
  }
} else {
  // 在非macOS平台提供空實現
  watchFrontmostApp = null;
